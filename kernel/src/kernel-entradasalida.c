...
void conexion_kernel_entradasalida() {
	...
	while (control) {
		...
		case FIN_IO:
			...
			log_info(kernel_logger, "Buscando la interfaz...");
			int fd_interfaz = buscar_socket_interfaz(listaInterfaces, nombre_interfaz);
			log_info(kernel_logger, "Se encontro la interfaz: %d", fd_interfaz);

			log_info(kernel_logger, "Recibiendo fin de io...");
			...
			log_info(kernel_logger, "Fin de io recibido");
			log_info(kernel_logger, "Copiando nombre...");
			strcpy(nombre_fin_io, nombre_fin_io_recibido);
			log_info(kernel_logger, "Nombre copiado");
